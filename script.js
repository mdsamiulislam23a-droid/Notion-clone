...
class NotionApp {
    ...
    createBlockElement(block) {
        ...
        if (editable && !isLocked) {
            ...
            editable.oninput = (e) => {
                ...
                // **NEW: Enhanced slash command detection**
                if (block.type === 'text' && editable.innerText === '/') {
                    this.openSlashMenu(wrapper, block.id);
                } else if (this.slashMenuOpen && !editable.innerText.includes('/')) {
                    this.closeSlashMenu();
                }
    ...
    openSlashMenu(blockWrapper, blockId) {
        this.slashMenuOpen = true;
        this.activeBlockIdForMenu = blockId;

        const menu = document.getElementById('slash-menu');

        // **FIX: Use absolute positioning so menu scrolls with content**
        menu.style.position = 'absolute';
        menu.style.display = 'flex';

        // Get the block wrapper's position relative to the document
        const wrapperRect = blockWrapper.getBoundingClientRect();
        const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
        const scrollLeft = window.pageXOffset || document.documentElement.scrollLeft;

        // Calculate absolute position (document coordinates)
        const top = wrapperRect.bottom + scrollTop + 4;
        const left = wrapperRect.left + scrollLeft;

        menu.style.top = top + 'px';
        menu.style.left = left + 'px';

        this.currentBlockForMenu = blockWrapper.dataset.id;
        this.slashMenuIndex = 0;
        this.highlightMenuItem(0);
    }

    closeSlashMenu() { this.slashMenuOpen = false; document.getElementById('slash-menu').style.display = 'none'; }
    navigateSlashMenu(key) {
        const items = document.querySelectorAll('#slash-menu .menu-item');
        if (key === 'ArrowDown') this.slashMenuIndex = (this.slashMenuIndex + 1) % items.length;
        else if (key === 'ArrowUp') this.slashMenuIndex = (this.slashMenuIndex - 1 + items.length) % items.length;
        else if (key === 'Enter') { items[this.slashMenuIndex].click(); return; }
        this.highlightMenuItem(this.slashMenuIndex);
    }
    highlightMenuItem(index) {
        document.querySelectorAll('#slash-menu .menu-item').forEach((el, i) => {
            if (i === index) { el.classList.add('active'); el.scrollIntoView({ block: 'nearest' }); } else el.classList.remove('active');
        });
    }

    convertBlock(type) {
        const page = this.getCurrentPage();
        const block = page.blocks.find(b => b.id === this.currentBlockForMenu);
        if (block) {
            if (type === 'page') {
                const newPage = this.addPage(this.activePageId); newPage.title = block.content.replace('/', '') || 'Untitled';
                block.type = 'page'; block.pageId = newPage.id; block.content = '';
            } else { block.type = type; block.content = block.content.replace('/', ''); if (type === 'todo') block.checked = false; }
            this.save(); this.loadPage(this.activePageId);
            setTimeout(() => { const el = document.querySelector(`[data-id="${block.id}"] [contenteditable]`); if (el) el.focus(); }, 0);
        }
        this.closeSlashMenu();
    }
     ...
     */
    turnBlockInto(blockId, newType) {
        const block = this.getCurrentPage().blocks.find(b => b.id === blockId);
        if (!block) return;
        ...
        if (newType === 'page') {
            const newPage = this.addPage(this.activePageId);
            newPage.title = oldContent || 'Untitled';
            block.type = 'page';
            ...
            if (newType === 'todo' && block.checked === undefined) {
                block.checked = false;
            }
        }
